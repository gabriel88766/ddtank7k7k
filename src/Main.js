	...
	loadFlash() {
		...
		switch (process.platform) {
			case "darwin":
				...
				break;
			case "linux":
				n = s.join(__dirname.includes(".asar") ? process.resourcesPath : __dirname, "../plugins/flash/linux/libpepflashplayer3200371.so"), r = "32.0.0.371";
				e.commandLine.appendSwitch('no-sandbox');
				break;
			default:
		...
		e.commandLine.appendSwitch("disable-renderer-backgrounding"), e.commandLine.appendSwitch("--ppapi-flash-path", n), e.commandLine.appendSwitch("--ppapi-flash-version", r), e.commandLine.appendSwitch('ignore-certificate-errors', 'true'), e.commandLine.appendSwitch('allow-insecure-localhost', 'true');
	}
	renderHome() {
