...
function removeTab(tabId) {
	...
	// fix current tab pointer if needed
	if (s === tabId) {
		s = null;
	}
}
	...
	return new Promise((l, o) => {
		if (e in n) l();
			...
			r.addEventListener('new-window', async (e) => {
				const oldTab = e;
				let tbnm = "tab" + Date.now().toString();
				createTab(tbnm, e.url, "DDTank", null).then(() => switchTab(tbnm, !0), removeTab(oldTab));
			});
...
async function init() {
	await createTab("index", url, "DDTank", null), setTimeout(() => switchTab("index", !1), 500)
}
